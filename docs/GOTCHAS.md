...
- **`CHUNK_TOO_LONG` almost never fires**: it's a Kokoro-only, ADVISORY backstop in `script_gen/linter.py` for a chunk `merge_sentences_to_chunks()` produces above `MAX_CHUNK_TOKENS` (150 tokens). It only runs when `check_format()`/`check()` is called with `engine="kokoro"` (default `None` skips it) — `auto_generate.py :: generate_script()` passes `engine=config.tts_engine`, so it's live in the production auto-generate flow for Kokoro configs (a no-op for F5 configs, same as before). Because the chunker flushes before exceeding the limit, only a single ~115+-word sentence can trip it — and a sentence that long has already tripped the far cheaper `SENTENCE_TOO_LONG` check (25-word threshold). Don't expect it to fire independently; it exists for the case that somehow slips past `SENTENCE_TOO_LONG`.
